...
# import gauss_psf_cuda_cameranerf as gauss_psf_cuda

# Misc
img2mse = lambda x, y: torch.mean((x - y) ** 2)
mse2psnr = lambda x: -10. * torch.log(x+0.00001) / torch.log(torch.Tensor([10.]))
to8b = lambda x: (255 * np.clip(x, 0, 1)).astype(np.uint8)

HALF_PIX = 0.5

def pixpos2AIFROI(camera_pix_pos,kernel_size,AIF_imgs):
    print("Loading AIF ROI")
    radius=kernel_size//2
    sh=AIF_imgs.shape
    pix_pos=camera_pix_pos.reshape(-1,2)
    pos_x, pos_y = pix_pos[:,0], pix_pos[:,1]

    AIF_ROIs={}
    for idx in range(sh[0]):
        print(idx)
        AIF_padding_ROIs = []
        AIF_img = np.transpose(AIF_imgs[idx], (2, 0, 1))
        AIF_padding = np.pad(AIF_img, ((0, 0), (radius, radius), (radius, radius)), 'constant')
        for i, (px, py) in enumerate(zip(pos_x, pos_y)):
            AIF_padding_ROI = AIF_padding[:, (py - radius) + radius:(py + radius + 1) + radius,
                              (px - radius) + radius:(px + radius + 1) + radius]
            AIF_padding_ROIs.append(AIF_padding_ROI)
        AIF_padding_ROIs = np.stack(AIF_padding_ROIs, axis=0)
        AIF_padding_ROIs = AIF_padding_ROIs.reshape(sh[1], sh[2], 3, kernel_size, kernel_size)
        AIF_ROIs[str(idx)]=AIF_padding_ROIs
    print("Loading end")
    return AIF_ROIs

def ray_show_img(ray,H,W,is_torch=False):
    if is_torch:
        img=torch.reshape(ray,(H,W,3))
        img=img.cpu().detach().numpy()
    else:
        img = np.reshape(ray, (H, W, 3))
...
def get_rays(H, W, K, c2w):
    ...
    j = j.t()
    dirs = torch.stack(
        [(i + (HALF_PIX - K[0][2])) / K[0][0], -(j + (HALF_PIX - K[1][2])) / K[1][1], -torch.ones_like(i)], -1)
...
def onepoint2kernelpoint_color(img, kernel_size):
    hei, wid = img.shape[0], img.shape[1]
    kc = np.zeros((img.shape[0], img.shape[1], img.shape[2], 9), dtype=np.float32)
    radius = kernel_size // 2

    img_pad = np.zeros([hei + radius * 2, wid + radius * 2, 3], dtype=np.float32)
    img_pad[radius:hei + radius, radius:wid + radius, :] = img

    for h in range(radius, hei + radius):
        for w in range(radius, wid + radius):
            kc[h - radius, w - radius, :, 0] = img_pad[h - radius, w - radius, :]
            kc[h - radius, w - radius, :, 1] = img_pad[h - radius, w, :]
            kc[h - radius, w - radius, :, 2] = img_pad[h - radius, w + radius, :]
            kc[h - radius, w - radius, :, 3] = img_pad[h, w - radius, :]
            kc[h - radius, w - radius, :, 4] = img_pad[h, w, :]
            kc[h - radius, w - radius, :, 5] = img_pad[h, w + radius, :]
            kc[h - radius, w - radius, :, 6] = img_pad[h + radius, w - radius, :]
            kc[h - radius, w - radius, :, 7] = img_pad[h + radius, w, :]
            kc[h - radius, w - radius, :, 8] = img_pad[h + radius, w + radius, :]
